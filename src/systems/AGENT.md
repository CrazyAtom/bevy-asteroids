...
- **`movement.rs`** — `FixedUpdate`에서 `apply_velocity`(속도 적분) · `apply_spin`(회전) · `wrap_around`(화면 순환, `Wrapping` 且 `Without<EdgeReflect>`) · `reflect_or_wrap`(`EdgeReflect` 대상: `StageModifiers.wall_bounce`면 벽 반사, 아니면 순환). `StageModifiers` 리소스(테마별 물리 트위스트: 벽 반사·우주선 미끄럼)도 여기 정의하고 기본값 삽입.
