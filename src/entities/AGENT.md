...
- **`player.rs`** (가장 큼) — 우주선. 입력·회전(램프)·추진/브레이크·화염(자식 스프라이트)·실드 스프라이트·연사/확산 타이머·`SpecialWeapon`(레이저빔)·하이퍼스페이스·감쇠·디버그 핫키(F1).
...
- **확장**: 새 특수무기는 `SpecialWeaponKind`(powerup.rs) 변형 + player의 발동 분기로, 새 보스는 `BossKind` 변형 + `boss_movement`/`boss_attack`의 arm 추가로.
