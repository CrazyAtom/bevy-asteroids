...
- **`state.rs`** — 상태 두 축: `GameState`(`Title`/`Playing`/`GameOver` + 재시작용 1프레임 바운스 상태)와 그 하위 `RunPhase`(`Running`/`Paused` — `Playing`에 진입하면 자동 `Running`). 리소스(`Score`/`Lives`/`HighScore`), `GameplayEntity` 마커, `GameStatePlugin`, `reset_game`(재시작 시 상태 초기화; `pub(crate)`).
